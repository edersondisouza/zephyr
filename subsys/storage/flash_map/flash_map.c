...
{
#ifdef CONFIG_FLASH_MAP_CUSTOM_BACKEND
	printk("flash_area_read %u - %d\n", fa->fa_id, HAS_CUSTOM_BACKEND_ID(fa->fa_id));
	if (HAS_CUSTOM_BACKEND_ID(fa->fa_id)) {
		printk("flash_area_read custom %p\n", fa->api);
		if (fa->api->read) {
			printk("flash_area_read custom read\n");
			return fa->api->read(fa, off, dst, len);
		}
		return -ENOTSUP;
	}
#endif
	printk("flash_area_read wrong!!\n");
