...
struct mctp_endpoint init_mctp_i2c_gpio(struct mctp_binding_i2c_gpio_controller *i2c_gpio)
{
	...
	endpoint.mctp_ctx = mctp_init();
	if (endpoint.mctp_ctx == NULL) {
		LOG_ERR("Failed to initialize MCTP context on i2c gpio");
		return endpoint;
	}
...
#if DT_NODE_HAS_STATUS_OKAY(DT_NODELABEL(mctp_i3c))
MCTP_I3C_CONTROLLER_DT_DEFINE(mctp_i3c_ctrl, DT_NODELABEL(mctp_i3c));

struct mctp_endpoint init_mctp_i3c(struct mctp_binding_i3c_controller *i3c)
{
	struct mctp_endpoint endpoint;

	endpoint.mctp_ctx = mctp_init();
	if (endpoint.mctp_ctx == NULL) {
		LOG_ERR("Failed to initialize MCTP context on i3c");
		return endpoint;
	}

	mctp_register_bus(endpoint.mctp_ctx, &i3c->binding, LOCAL_EID);
	mctp_set_rx_all(endpoint.mctp_ctx, rx_message, NULL);

	endpoint.eid = i3c->endpoint_ids[0];
...
int main(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(endpoints); i++) {
		if (endpoints[i].mctp_ctx != NULL) {
			probe(endpoints[i].mctp_ctx, endpoints[i].eid);
		}
	}
