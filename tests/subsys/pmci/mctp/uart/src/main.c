...
static void target_reply(struct k_work *item)
{
	...
	rc = mctp_message_tx(reply->mctp_ctx, reply->eid, false, 0, reply->reply_msg,
			     reply->reply_len);
...
ZTEST(mctp_uart_test_suite, test_mctp_uart_ping_pong_big_message)
{
	...
	/* A string with 513 chars*/
	rc = mctp_message_tx(mctp_ctx, ENDPOINT_EID, false, 0, big_message, sizeof(big_message));
	zassert_ok(rc, "Failed to send message");
